...
const compiledRelatedListTemplate = Handlebars.compile(relatedListTemplate.innerHTML);
const compiledRelatedNodeTemplate = Handlebars.compile(relatedNodeTemplate.innerHTML);
...
function messagesFromArray(nodes) {
    let list = nodes.map(function(failure) {
        return {
...
function messageFromRelatedNodes(relatedNodes) {
    ...
    if (relatedNodes.length) {
        let list = relatedNodes.map(function(node) {
            return {
...
exports.printReport = function generateHTML(results, filename) {
    ...
    results.forEach(function(page) {
        if (page.result.violations.length) {
            let violations = page.result.violations.map(function(rule, i) {
                return {
                    impact: rule.impact,
                    help: rule.help,
    ...
    document.getElementById('reportSection').innerHTML = compiledPageListTemplate({pageList: pageList});
    const data = dom.serialize();

    fs.writeFile(path.join(__dirname, filename + '-report.html'), data, 'utf8', (err) => {
        if (err) throw err;

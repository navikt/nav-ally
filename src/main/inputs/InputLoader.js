...
function loadInputFromEnvironment(definitionFile, yamlFile) {
    ...
    error('Use the environment variable DEFINITION_FILE to load input from file.');
    process.exit(1);
}
...
function handleInputFromDefinitionFile(definitionFile) {
    ...
    if (!definitionFile) {
        trace("Invalid input. Did you forget to set the DEFINITION_FILE environment variable?");
        process.exit(1);
    }
